...
public class User {
    ...
    public static List<User> filterZhangUsers(List<User> users) {
        return filterUserByLastName(users, "张");
    }

    // 过滤姓王的用户
    public static List<User> filterWangUsers(List<User> users) {
        return filterUserByLastName(users, "王");
    }

    public static List<User> filterUserByLastName(List<User> users, String lastName) {
        return filter(users, new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.name.startsWith(lastName);
            }
        });
    }
