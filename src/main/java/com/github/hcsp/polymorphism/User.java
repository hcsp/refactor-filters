...
public class User {
    ...
    // 并简化上面三个函数
    public static List<User> filter(List<User> users, Predicate<User> mycondition) {
        List<User> results = new ArrayList<>();
        for (User user : users) {
            if (mycondition.test(user)) {
                results.add(user);
            }
        }
        return results;
  ...
  /*  public static interface MyCondition {
        boolean MyConditionistrue (User user);
    }*/
