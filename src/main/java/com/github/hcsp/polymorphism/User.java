...
//    private interface 判断条件 {
//        boolean 是否满足条件(User user);
//    }
//
//    private static class 判断是否姓王 implements 判断条件 {
//        @Override
//        public boolean 是否满足条件(User user) {
//            return user.name.startsWith("王");
//        }
//    }
//    public static List<User> filter(List<User> users, 判断条件 条件) {
//        List<User> results = new ArrayList<>();
//        for (User user : users) {
//            if (条件.是否满足条件(user)) {
//                results.add(user);
...
//    }
    ...
    // 过滤ID为偶数的用户
//    public static List<User> filterUsersWithEvenId(List<User> users) {
//        return filter(users, (判断条件) user -> user.id % 2 == 0);
//    }
//    // 过滤姓张的用户
//    public static List<User> filterZhangUsers(List<User> users) {
//        return filter(users, (判断条件) user -> user.name.startsWith("张"));
//    }
//    // 过滤姓王的用户
//    public static List<User> filterWangUsers(List<User> users) {
//        return filter(users,new 判断是否姓王());
//    }
}
